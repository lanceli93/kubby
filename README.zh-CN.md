...
受 Jellyfin 启发，用现代技术栈重新构建的自托管个人影音服务器。基于 Next.js。Kubby 覆盖四个媒体域——**电影**、**剧集与番剧**、**照片**、**音乐**——每个域都有独立的媒体库、扫描器和浏览体验，共享同一套暗色影院主题。
...
把 Kubby 指向一个剧集或番剧文件夹，它会解析 `Season NN`/`Specials` 文件夹和 `SxxExx` 文件名，构建一个三层媒体库——剧集 → 季 → 单集——并从 `tvshow.nfo`/`episodedetails` 和 TMDB 的 `/tv` 接口刮削海报、单集剧照和演员信息。它与电影侧共用同一套暗色主题、卡片、演员页、多维度评分、Cover Flow 海报墙和视频播放器；剧集独有的部分就是下面这些。

### 按季浏览单集

剧集详情页以一张影院式头图开场，然后按季拆分整部剧：在各季之间切换标签，每一集都列出自己的剧照、标题、时长和播放按钮。观看状态和续播进度是**按单集**记录的，而不是按整部剧。

![剧集详情页，按季展示单集](docs/screenshots/tv-detail.webp)

### 播放列表（Next Up）

剧集主页有一条 **继续观看 / Next Up** 栏：看完一集后，Kubby 会在你所有剧集里推出下一集，看到一半的还会带上进度条——总能一眼找到接下来该看什么。
...
电影、剧集、照片、音乐各有自己的主页和导航，但共用同一套账户、主题和图片管线。两处域切换器可在它们之间跳转——Kubby 品牌处的下拉菜单，以及左侧抽屉的「媒体」分组——并记住上次使用的域。无论你浏览哪个域，全局音乐播放器都会持续播放。

![左侧抽屉域切换器，完整列出四个媒体域](docs/screenshots/domain-switcher.webp)
...
Kubby 没有用 Apple Developer ID 签名，macOS 下载后会给它打上隔离标记。只需去掉一次隔离标记，之后就能一直正常打开：

```bash
xattr -dr com.apple.quarantine /Applications/Kubby.app
```

这是目前所有 macOS 版本上都可靠的办法。执行完这条命令后，像平常一样双击打开 Kubby 即可。

> **注意：** 老办法"右键 → 打开" / "仍要打开"在较新的 macOS（Sequoia 及以后）对未公证的应用已经失效——你往往只会看到 *"Kubby 已损坏，无法打开"*，连绕过按钮都没有。请改用上面的 `xattr` 命令。
...
| 状态 | 用户体验 |
|------|---------|
| **未签名**（目前） | 被拦成"已损坏"/"无法验证开发者"，用 `xattr -dr com.apple.quarantine /Applications/Kubby.app` 去掉一次隔离标记即可 |
