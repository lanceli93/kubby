...
- **Every route 500s on user machines with `Cannot find module 'better-sqlite3-<hash>'`**: Turbopack emits `serverExternalPackages` (better-sqlite3, sharp) as `.next/node_modules/<pkg>-<hash>` **symlinks** pointing at the build host's `node_modules` (outside the bundle). Two things bake an absolute build-machine path (e.g. `/Users/runner/work/kubby/...`) into the shipped bundle: (1) copying those links verbatim from standalone, and (2) `fs.cpSync`'s default `verbatimSymlinks:false`, which re-absolutizes relative links on *every* copy — including the DMG-staging copy. Fix (in `package.ts`): `fixStandaloneSymlinks()` repoints each link to the in-bundle relative target `../../node_modules/<pkg>`, and `copyDirRecursive()` passes `verbatimSymlinks:true` so those relative targets survive both the assemble copy and the DMG-staging copy. **Invariant:** no symlink inside the shipped `.app` may resolve to an absolute `/Users/...` path — verify with `find Kubby.app -type l` + `readlink`.
- **Unsigned `.app` won't open on macOS**: the app is only ad-hoc signed (no Developer ID / notarization), so on Sequoia+ users get "damaged / can't be opened" with no bypass button. The reliable fix is `xattr -dr com.apple.quarantine /Applications/Kubby.app` (documented in both READMEs). "Right-click → Open" is dead for un-notarized apps on current macOS.
