...
│   │   │   ├── personal-metadata/page.tsx          # 个人元数据设置 (评分维度/卡片标记)
...
│   ├── hooks/
│   │   └── use-user-preferences.ts                 # 用户偏好 React Query hook (评分维度/卡片标记)
│   ├── providers/
...
| `/api/settings/personal-metadata` | GET/PUT | 用户偏好设置 (评分维度/卡片标记) |
...
| `AdminSidebar` | `components/layout/` | 管理侧边栏: 概览/媒体库/用户/刮削器, 蓝色左边框高亮 |
