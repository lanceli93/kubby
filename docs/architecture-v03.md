...
> 本地自托管影音管理系统，参考 Jellyfin 架构简化实现。当前版本支持**电影**、**照片**（手机照片+视频，含时间线 + 手动相册）与**音乐**（专辑/艺术家/歌曲 + 全局常驻播放器）三个媒体域：域分离架构（`docs/photos-library-design.md`、`docs/music-library-design.md`），每域独立表/扫描器/页面，共享库管理、播放管线、认证、i18n。影院域 UI 不显示照片库（首页媒体库/搜索/海报墙偏好均按 `type !== "photo"` 客户端过滤）。音乐域引入唯一的跨路由常驻播放器（`music-player-provider` + 底部 `NowPlayingBar`）。
...
│   │   ├── db/
│   │   │   ├── schema.ts                           # Drizzle schema (22 张表, 含 settings + user_preferences + bookmarks + photo_items + photo_albums/photo_album_items + music_* 6 表)
...
media_libraries ──1:N──> movies ──1:N──> movie_people ──N:1──> people
                  ...
                  │                ├──1:N──> movie_discs
                  │                └──1:N──> media_streams
                  ├──1:N──> photo_items (照片域, 与 movies 平行) <──N:M──> photo_albums
                  ├──1:N──> photo_albums ──1:N──> photo_album_items ──N:1──> photo_items
                  └──1:N──> music_albums ──1:N──> music_tracks   (音乐域, 与 movies 平行)
                            │                        │
                            └─M:N─ music_album_artists │  music_track_artists ─M:N─┐
                                            └──> music_artists <────────────────────┘
                                                     (user_track_data ──N:1──> music_tracks)
...
| folder_path | text | 相对库根路径 (扫描来源目录; 相册与之无关, 见下) |
| date_added / date_modified | — | 入库时间 / 文件 mtime ms (增量扫描比对键) |

**索引**: library_id, (library_id, taken_at), folder_path, is_video

#### photo_albums (照片相册 — 用户手动分类, 非扫描文件夹)
| 列 | 类型 | 说明 |
|----|------|------|
| id | text PK | UUID |
| library_id | text FK | → media_libraries, CASCADE (相册归属某个照片库) |
| name | text | 相册名 |
| cover_item_id | text | 封面 photo_items.id; 为空或已不是成员时回退为最新成员 |
| sort_order | integer | 排序 (默认 0) |
| created_at | text | 时间戳 |

**索引**: library_id

> **相册是用户手动创建的分类, 不是扫描文件夹自动生成的** (用户明确要求)。默认无相册, 就是一条时间线; `folder_path` 只记扫描来源目录, 与相册无关。一张照片可属于多个相册。

#### photo_album_items (相册成员 join)
| 列 | 类型 | 说明 |
|----|------|------|
| album_id | text FK | → photo_albums, CASCADE |
| item_id | text FK | → photo_items, CASCADE |
| added_at | text | 加入时间 |

**索引**: UNIQUE(album_id, item_id) — 重复添加 no-op (`onConflictDoNothing`); item_id。删除相册/相册成员均不影响底层照片。
...
| `/api/photos` | GET | 照片时间线 cursor 分页 (`?cursor={takenAt}_{id}&limit=&libraryId=&albumId=`, takenAt DESC + id tiebreak, limit 默认 100 上限 500; albumId 用子查询限定成员) |
...
| `/api/photos/albums` | GET/POST | 相册列表 (`?libraryId=`, 带成员数 + 解析封面) / 新建相册 (name+libraryId, 仅照片库) |
| `/api/photos/albums/[id]` | GET/PATCH/DELETE | 单相册头 / 重命名·设封面 / 删除 (成员关联级联, 底层照片保留) |
| `/api/photos/albums/[id]/items` | POST/DELETE | 加入照片 (itemIds, INSERT OR IGNORE, 仅同库照片) / 移出照片 |
...
| `/photos` | 照片域首页 | 顶部「时间线 \| 相册」分段控件 + 库筛选下拉 (仅 >1 照片库时显示) + 多选模式 (批量加入相册)。**时间线** (`components/photos/photo-grid.tsx`, 时间线与相册详情共用): 按月分组 justified 等高行网格 (`lib/photos/justified-layout.ts` 纯函数) + `@tanstack/react-virtual` 行级虚拟滚动 + cursor 无限加载 (`useInfiniteQuery`, queryKey 按 `{libraryId, albumId}` 分域, limit 200) + 瓦片 hover 内缩放/内嵌 ring/底部渐变露拍摄日期 (瓦片本身不放大以免撑破 justified 行) + 视频时长角标/▶。**相册** (`albums-view.tsx`): 封面+数量卡 + 新建相册。与影院域共用暗色主题 |
| `/photos/view/[id]` | 照片灯箱 | `fixed inset-0 z-50 bg-black` 全屏: ←/→/滑动切换 (queryKey 与 photo-grid 一致——`?lib=`/`?album=` 参数携带作用域, 复用缓存找相邻项, router.replace 不堆历史), 滚轮缩放 0.5×–5× / 双击 2× / 拖拽平移, ⊞ 加入相册, ⓘ EXIF 面板 (拍摄时间/相机/尺寸/大小/GPS + 下载原件), 视频内嵌播放 (`lightbox-video.tsx`: 视频铺满舞台 + spinner 直到首帧, decide→direct 或 hls.js/原生 HLS, 卸载时 DELETE session), 相邻图片预加载, 清晰缩略图秒显打底 + 全图柔和交叉淡入 (无糊图硬跳) |
| `/photos/album/[id]` | 相册详情 | 复用 `PhotoGrid` (albumId 作用域) 的 justified 网格 + 头部重命名/删除相册 + 多选移出相册; 相册删除仅删分类, 底层照片保留 |
