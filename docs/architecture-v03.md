...
> 本地自托管影音管理系统，参考 Jellyfin 架构简化实现。当前版本支持**电影**与**照片**（手机照片+视频，含时间线 + 手动相册）两个媒体域：域分离架构（`docs/photos-library-design.md`），每域独立表/扫描器/页面，共享库管理、播放管线、认证、i18n。影院域 UI 不显示照片库（首页媒体库/搜索/海报墙偏好均按 `type !== "photo"` 客户端过滤）。
...
│   │   ├── db/
│   │   │   ├── schema.ts                           # Drizzle schema (16 张表, 含 settings + user_preferences + bookmarks + photo_items + photo_albums/photo_album_items)
