    ...
    >
      ...
      <div className={`relative w-full overflow-hidden rounded-md bg-[var(--surface)] ring-1 ring-white/[0.06] ${responsive ? "aspect-[2/3]" : ""}`} style={responsive ? undefined : { height: 270 }}>
        ...
        {showResBadge && (() => {
          ...
          return res ? (
            <div className="absolute left-1.5 top-1.5 rounded bg-white/30 backdrop-blur-md px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-black/80 shadow-sm">
              {res}
            </div>
          ) : null;
        })()}
