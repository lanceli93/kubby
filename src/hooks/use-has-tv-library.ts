...
// whether a "tvshow" library exists — not the full library payload (cover
...
// long staleTime keeps it cheap on domains that never touch tv.
export function useHasTvLibrary() {
  ...
  return data?.some((lib) => lib.type === "tvshow") ?? false;
}
