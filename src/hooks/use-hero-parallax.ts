...
export function useHeroParallax({ disabled, ready = true }: UseHeroParallaxOptions = {}) {
  ...
  // Latest scroll offset + pointer drift, consumed by a single scheduled rAF.
  const scrollYRef = useRef(0);
  const pointerRef = useRef<{ dx: number; dy: number }>({ dx: 0, dy: 0 });
  ...
  const applyFrame = () => {
    ...
    if (fanart) {
      ...
      fanart.style.transform = `translate3d(${-dx * FANART_DRIFT}px, ${
        scrollY * SCROLL_FACTOR - dy * FANART_DRIFT
      }px, 0)`;
    }
    const poster = posterRef.current;
    if (poster) {
      // Drift with the pointer (same direction, smaller magnitude).
      poster.style.transform = `translate3d(${dx * POSTER_DRIFT}px, ${
        dy * POSTER_DRIFT
      }px, 0)`;
    }
  };
  ...
  const reset = () => {
    ...
    pointerRef.current = { dx: 0, dy: 0 };
    const fanart = fanartRef.current;
    if (fanart) {
      fanart.style.transition = RESET_TRANSITION;
      fanart.style.transform = "";
    }
    const poster = posterRef.current;
    if (poster) {
      poster.style.transition = RESET_TRANSITION;
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const fanart = fanartRef.current;
      if (fanart) fanart.style.transition = "none"; // follow scroll without lag
      schedule();
    };
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const fanart = fanartRef.current;
      if (fanart) fanart.style.transition = "none";
      const poster = posterRef.current;
      if (poster) poster.style.transition = "none";
      schedule();
    };
    const onLeave = () => {
      pointerRef.current = { dx: 0, dy: 0 };
      const fanart = fanartRef.current;
      if (fanart) fanart.style.transition = RESET_TRANSITION;
      const poster = posterRef.current;
      if (poster) poster.style.transition = RESET_TRANSITION;
      schedule();
    };
