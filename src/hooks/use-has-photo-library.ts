...
// whether a "photo" library exists — not the full library payload (cover
...
// long staleTime keeps it cheap on domains that never touch photos.
export function useHasPhotoLibrary() {
  ...
  return data?.some((lib) => lib.type === "photo") ?? false;
}
